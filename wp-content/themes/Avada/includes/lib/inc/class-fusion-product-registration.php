...
class Fusion_Product_Registration {
	 ...
	 * @return bool
	 */
	public function is_registered( $product_id = '' ) {return true;
		if ( ! $product_id ) {
		...
		<div class="fusion-library-important-notice registration-form-container">
			...
			<div class="fusion-library-registration-form">
				<form id="fusion-library_product_registration" method="post">
					...
					<?php endif; ?>
					<input <?php echo ( ! $show_form ) ? 'class="toggle-hidden hidden" ' : ''; ?>type="text" name="<?php echo esc_attr( "{$this->option_name}[{$this->product_id}][token]" ); ?>" value="Licensed by plugintheme" />
